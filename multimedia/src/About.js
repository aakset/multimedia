...
function About() {
  return (
    <div className="container-about">
      <h1 className="titleAbout">About Northlake Studios</h1>
      ...
      <div className="audio-container">
        {[pic1, pic2, pic3].map((pic, index) => (
          <div className="audio-player" key={index}>
            <audio controls>
              <source src={require(`./Song${index + 1}.mp3`)} type="audio/mpeg" />
              Your browser does not support the audio element.
            </audio>
            <img src={pic} alt={`Image ${index + 1}`} className="audio-image" />
          </div>
        ))}
      </div>
    </div>
